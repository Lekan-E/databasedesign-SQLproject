/* USE Players
passing data can be found in the 'passingstats230530.csv file*/
USE Players;

SELECT club_shirt, 
       season, 
       attempted_pass AS total_passes, 
       CONCAT(ROUND((completed_passes/attempted_pass)*100,1),'%') as completed_pass,
       key_passes, 
       touches, 
       CONCAT(ROUND((attacking_3rd/touches)*100,1),'%') as touches_attack_3rd,
       goal_creating_actions AS GCA, 
       shot_creating_action AS SCA
FROM passing;
...
SELECT club_shirt, 
       season, 
       attempted_pass AS total_passes, CONCAT(ROUND((completed_passes/attempted_pass)*100,1),'%') as completed_pass,
       key_passes, 
       touches, 
       CONCAT(ROUND((attacking_3rd/touches)*100,1),'%') as touches_attack_3rd,
       goal_creating_actions AS GCA, shot_creating_action AS SCA
...
JOIN football_info fi ON fi.club_shirt = p1.CLUB_SHIRT
WHERE p1.season = '2021-2022' AND p2.season = '2022-2023'
GROUP BY p1.club_shirt;
